...
> ([Windows](LAB-19-WINDOWS.md) · [macOS](LAB-19-MACOS.md)). In class, follow **[STEPS.md](STEPS.md)** then the
...
> **Incremental build:** Test pyramid note → Boot IT → PostgreSQL strategy → Angular selectors → Selenium flow → GitHub
> Actions headless stage.

> **Critical scope:** Backend IT with **PostgreSQL** test strategy (Testcontainers *or* dedicated test DB — document
> which). UI automation targets **Angular** (not React). CI notes use **GitHub Actions**. Correlation `lab-request-001` on
> API calls.
...
This Module 19 lab connects the pyramid: Spring Boot integration tests for the REST Customer API, a clear **PostgreSQL**
test-data strategy, and Selenium WebDriver flows against the **Angular** CRM UI, with notes for running headless in
**GitHub Actions**.
...
**Every merge needs Boot IT against a PostgreSQL test strategy and at least one Selenium path on the Angular UI for
Amina/Ravi.**
...
**If it fails:** Driver mismatch → let Selenium Manager resolve; UI not up → start `ng serve` first. Element not found →
verify `data-testid`.
...
**Do this:** Capture Surefire output + one Selenium run (or failure screenshot policy). Confirm fixtures and
`lab-request-001` appear in API IT.
