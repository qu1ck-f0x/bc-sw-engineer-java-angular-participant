...
**Other
OS:** [Windows guide](LAB-26-WINDOWS.md) · [IDE conventions](../../../Week%201%20-%20Java%20and%20JVM%20Foundations/_IDE-CONVENTIONS.md)
...
| Evidence / screenshots  | `~/java-bootcamp/notes/screenshots/lab-26` |
...
mkdir -p notes/screenshots/lab-26
...
Re-verified instructor laptop (2026-08-07, Windows PowerShell; same Boot 3.3.5 / H2-only classpath): with `prod` active
and secrets unset, failure is **`Failed to load driver class org.postgresql.Driver`** (not always a “Could not resolve
placeholder” line). YAML `${DB_PASSWORD}` / `${NORTHSTAR_API_KEY}` without defaults remain env-only; see GUIDE Step 4.
...
Save under `~/java-bootcamp/notes/screenshots/lab-26`. Capture IntelliJ (project tree + Run/Terminal). Redact secrets.
