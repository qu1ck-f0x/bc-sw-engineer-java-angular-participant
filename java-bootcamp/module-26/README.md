...
**Critical:** Split **dev / test / prod** YAML. Activate via **`-D`** and **`SPRING_PROFILES_ACTIVE`**. Know override
order. Bind with **`@ConfigurationProperties`**. **Never commit real secrets** — `.env.example` placeholders only.
`prod` fails fast when required env vars are missing. Keep CRM fixtures working under `dev`.
