...
**Critical:** `@Transactional` on **TransferService** (not controller). Debit + credit + log = **one** unit of work.
Prove rollback with **`ACC-FORCE-FAIL`**. ACID notes cite **evidence**. Reject AI drafts that swallow exceptions or put
TX on controllers. JWT → Lab 28.
