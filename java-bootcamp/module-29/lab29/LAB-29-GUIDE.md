...
> finished Module 29 [pre-lab exercises 1–6](../exercises/EXERCISES-INDEX.md) (Pass in your notes; order **1 → 2 → 3 → 4 →
6 → 5**). Then open **one** OS how-to ([Windows](LAB-29-WINDOWS.md) · [macOS](LAB-29-MACOS.md)). In class, prefer the
...
> **Incremental build:** DTO constraints → handler TODOs → envelope → status map → MockMvc body plan → readiness → Lab
29.

> **Classroom pacing:** [`../PACING.md`](../PACING.md) (Checkpoints A–D).

> **Critical scope:** **`@Valid` + ErrorResponse**. Map **400/404/409**. Keep **`lab-request-001`**. Assert status
> **and** body. **No stack-trace HTML**. **Lab 28 security is already in the starter** — do not rebuild it. Focus on
...
3. **Security is provided** (Lab 28 packages: `SecurityConfig`, `JwtService`, filter, `AuthController`,
   `CrmUserDetailsService`). Fill validation TODOs only: `CustomerRequest` annotations, `@Valid` on create,
   `GlobalExceptionHandler` bodies.
...
This Module 29 lab unifies **Bean Validation** on request DTOs with a global `@RestControllerAdvice` that returns a
consistent **`ErrorResponse`** for REST failures. The starter already includes the **Lab 28 security baseline** — your
job is the validation and error-envelope TODOs, not rebuilding JWT.
...
Prior
labs: [14](../../../Week%202%20-%20Backend,%20AI%20Tools%20and%20Testing/module-14/lab14/LAB-14-GUIDE.md) · [16](../../../Week%202%20-%20Backend,%20AI%20Tools%20and%20Testing/module-16/lab16/LAB-16-GUIDE.md) · [25](../../module-25/lab25/LAB-25-GUIDE.md) · [28](../../module-28/lab28/LAB-28-GUIDE.md).
...
Complete each step in order. Commands assume `~/java-bootcamp/examples/lab29-crm` (Windows:
`%USERPROFILE%\java-bootcamp\examples\lab29-crm`) unless noted.
...
**Do this:** Fill starter TODOs. Build `ErrorResponse` with existing setters; map field errors to
`FieldViolation(field, message)` only:
...
**Expected result:** Re-run invalid POST; JSON matches `ErrorResponse`; `correlationId` reflects `lab-request-001` when
header sent.
...
Fill `docs/error-contract.md` (not `error-contract-notes.md`) with the status table and a note that customer APIs
require Lab 28 Bearer auth.
...
**Do this:** Starter has **0** tests. Add `com.northstar.crm.ErrorEnvelopeTest` with login helper (`agent1`/`agent1`)
and Bearer header:
...
**Do this:** Complete Failure Experiments. Add a short paragraph in `docs/error-contract.md` stating how Lab 14 DTO
constraints and Lab 16 handlers are now one Boot contract. Capture curl bodies under `notes/screenshots/lab-29/`.
