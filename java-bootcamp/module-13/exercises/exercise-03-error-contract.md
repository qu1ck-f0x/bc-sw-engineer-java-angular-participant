...
Without one error shape, every client writes bespoke parsing for every endpoint, and support cannot trace a reported
failure.
...
One error envelope with a code and correlation id, two worked examples, and a leak rule in
`notes/lab13-error-contract.md`.
